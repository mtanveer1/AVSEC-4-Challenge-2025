...
def subsample_list(inp_list: List, sample_rate: float) -> List:
    """
    Retruns a randomly subsampled list of items while retaining a fraction of the items.
    """
...
class AVSE4Dataset(Dataset):
    """
    AVSE dataset for loading and preprocessing audio-visual pairs.
    Supports both training (with clean targets) and evaluation (without targets).
    """
    ...
    def build_files_list(self) -> List[Tuple[str, str, str, str, str]]:
        """
        Builds list of file paths for all available scenes.
        """
    ...
    def get_files_list(self, scenes_root: str) -> List[Tuple[str, str, str, str, str]]:
        """
        Get list of file paths for a single scenes directory.
        """
        files_list = []
        ...
        if self.test_set:
            ...
            for file in os.listdir(scenes_root):
                if file.endswith("_target_anechoic.wav"):
                    files = (
                        join(scenes_root, file),  # clean audio
                        join(scenes_root, file.replace("target_anechoic", "interferer")),  # noise
                        join(scenes_root, file.replace("target_anechoic", "mono_mix")),    # noisy audio
                        join(scenes_root, file.replace("target_anechoic.wav", "silent.mp4")),  # video
                        join(scenes_root, file.replace("target_anechoic", "mix")),  # dummy
                    )
    ...
    def __getitem__(self, idx):
        """
        Load and preprocess a single audio-visual sample.
        """
        clean_path, _, mix_path, video_path, _ = self.files_list[idx]

        video_tensor = self._load_video(video_path)  # [T, 3, H, W] or [1, T, H, W]
        noisy_audio, _ = torchaudio.load(mix_path)   # [C, N] or [N]
        
        # Handle target audio based on dataset mode
        if self.test_set:
            clean_audio = torch.zeros_like(noisy_audio)

        else:
            clean_audio, _ = torchaudio.load(clean_path)  # [C, N] or [N]
        ...
        if self.mono:
            ...
            if noisy_audio.dim() == 1:
                noisy_audio = noisy_audio.unsqueeze(0).repeat(2, 1)  # Convert mono to stereo
        ...
        if self.mono:
            ...
            if clean_audio.dim() == 1:
                clean_audio = clean_audio.unsqueeze(0).repeat(2, 1)  # Convert mono to stereo
        ...
        def pad_or_truncate_video(video, max_frames=75):
            """Pad or truncate video to fixed number of frames."""
            T = video.shape[0]
            if T < max_frames:
        ...
        if self.rgb:
            video_tensor = video_tensor.transpose(1, 2).transpose(2, 3)  # [T, 3, H, W] -> [T, H, W, 3]
        else:
            video_tensor = video_tensor.transpose(0, 1).transpose(1, 2)  # [1, T, H, W] -> [T, H, W, 1]
    ...
    def _load_video(self, video_path: str) -> torch.Tensor:
        ...
        # Format based on RGB/grayscale mode
        if not self.rgb:
            frames = frames[np.newaxis, ...]  # [1, T, H, W] for grayscale
        else:
            frames = frames.transpose(0, 3, 1, 2)  # [T, C, H, W] for RGB
            
        return torch.from_numpy(frames).float()

    def _process_frames(self, frames: np.ndarray) -> np.ndarray:
        """
        Process video frames: crop, resize, and normalize.
        
        Args:
            frames: Raw video frames [T, H, W, C]
            
        Returns:
            Processed frames [T, H, W, C] or [T, H, W] for grayscale
        """
        # Crop frames to center region
        ...
        # Pad to maximum frame count if needed
...
class AVSE4DataModule(LightningDataModule):
    """
    PyTorch Lightning DataModule for Audio-Visual Speech Enhancement.
    
    Manages training, validation, and evaluation datasets with appropriate
    data loaders and preprocessing configurations.
    """
    def __init__(self, data_root, batch_size=16, audio_norm=False, rgb=True, num_channels=1):
        super().__init__()
        self.batch_size = batch_size
        
        # Training dataset with fixed-length batches
        ...
        # Validation dataset with fixed-length batches
        ...
        # Evaluation dataset with full-length sequences
    ...
    def train_dataloader(self):
        """DataLoader for training with shuffling and multi-worker loading."""
    ...
    def val_dataloader(self):
        """DataLoader for validation with multi-worker loading."""
    ...
    def test_dataloader(self):
        """DataLoader for evaluation/testing."""
        return torch.utils.data.DataLoader(
            self.eval_dataset, 
            batch_size=self.batch_size, 
            num_workers=2
